# Documentación Técnica: Proyecto Integrador de Fin de Asignatura
...
**Propósito:** Plataforma web (ABM/CRUD) diseñada para gestionar ofertas laborales, postulaciones y perfiles de estudiantes y empresas, enfocada en la Universidad Católica Boliviana.
...
*   **Usuarios:** Diferenciación por roles (Admin, Company, Student).
...
*   *(Añadir aquí el enlace al diagrama de base de datos o un resumen más extenso si se desea)*.
...
*   **Delete:** Eliminación o archivado de ofertas (soft delete).
...
> **Nota para el estudiante:** Aquí debes insertar un diagrama lógico detallado usando draw.io, Lucidchart o similar con íconos oficiales (GitHub, Vercel, Render, Docker).
...
4.  **Hosting del Backend & Base de Datos:** Desplegado en **Render**. Render expone el servicio web (NestJS) a través de su balanceador de carga y provisiona el servicio gestionado de PostgreSQL.
...
> **Nota para el estudiante:** Inserta aquí una pequeña tabla o capturas de pantalla de los paneles de "Billing" o "Usage" de Vercel y Render mostrando que te encuentras dentro del Free Tier o mostrando los costos estimados reales.
...
*   **Pilar 1 (Docker):** (Adjuntar captura del comando `docker-compose ps` mostrando contenedores healthy y el endpoint de healthcheck respondiendo).
*   **Pilar 3 (CI/CD):** (Adjuntar captura de los Actions de GitHub ejecutándose con éxito (checkmarks verdes), mostrando el job de Unit Tests y el de Deploy).
